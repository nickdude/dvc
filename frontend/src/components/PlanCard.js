...
const PlanCard = ({ title, price, subtitle, onContinue, disabled }) => {
    return (
        <div className="flex items-start flex-col justify-center bg-white shadow-[0px_0px_8px_0px_#1D1D1F08] p-6 rounded-2xl w-[294px] gap-6 equal-height-card h-full">
            ...
            <p className="text-base text-semiLightGrey">
                {price === "0" ? "Free Trial" : `Equal to $${(price / 12).toFixed(2)}/mo`}
            </p>
